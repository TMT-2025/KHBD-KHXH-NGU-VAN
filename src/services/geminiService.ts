...
export async function integrateNLS(content: string, subject: string, grade: string) {
  ...
  const prompt = `
    ...
    3. Trong "III. TIẾN TRÌNH DẠY HỌC":
       ...
       - QUY TẮC BẮT BUỘC CHO NHIỀU TIẾT: Đối với bài dạy nhiều tiết (ví dụ: bài học 2, 3, 4 tiết...), bạn phải thực hiện thiết kế chi tiết tất cả các hoạt động cho từng tiết (Ví dụ: Tiết 1 gồm Hoạt động 1, Hoạt động 2; Tiết 2 gồm Hoạt động 3, Hoạt động 4...). TUYỆT ĐỐI không tóm tắt sơ sài hay gom cụm các tiết sau. TẤT CẢ các hoạt động học ở tất cả các tiết (từ Tiết 1 đến Tiết cuối cùng) đều PHẢI ĐƯỢC THIẾT KẾ ĐẦY ĐỦ VÀ CHI TIẾT THEO CẤU TRÚC 4 PHẦN (a, b, c, d) DƯỚI ĐÂY.
